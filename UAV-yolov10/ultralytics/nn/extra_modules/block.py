...
__all__ = ['DyHeadBlock', 'DyHeadBlockWithDCNV3', 'Fusion', 'C2f_Faster', 'C3_Faster', 'C3_ODConv', 'C2f_ODConv', 'Partial_conv3', 'C2f_Faster_EMA', 'C3_Faster_EMA', 'C2f_DBB',
           ...
           'BiFusion', 'RepBlock', 'C3_EMBC', 'C2f_EMBC', 'SPPF_LSKA','SPPF_SEAttention' ,'SPPF_EMA','SPPF_DRB',
           'C3_DAttention', 'C2f_DAttention', 'C3_Parc', 'C2f_Parc', 'C3_DWR', 'C2f_DWR',
...
######################################## SPPF with LSKA end ########################################
######################################## SPPF with SPPF_SEAttention start ########################################

class SPPF_SEAttention(nn.Module):
    """Spatial Pyramid Pooling - Fast (SPPF) layer with SEAttention for YOLOv5."""
    ...
    def __init__(self, c1, c2, k=5, reduction=16):  # equivalent to SPP(k=(5, 9, 13))
        ...
        self.cv2 = Conv(c_ * 4, c2, 1, 1)
        self.m = nn.MaxPool2d(kernel_size=k, stride=1, padding=k // 2)
        self.se = SEAttention(c_ * 4, reduction)  # Initialize SEAttention with correct channels
    ...
    def forward(self, x):
        ...
        y = self.se(y)  # Apply SEAttention
        return self.cv2(y)

######################################## SPPF with SPPF_SEAttention end ########################################
######################################## SPPF with SPPF_EMA start ########################################

class SPPF_EMA(nn.Module):
    """Spatial Pyramid Pooling - Fast (SPPF) layer with EMA for YOLOv5."""

    def __init__(self, c1, c2, k=5, factor=8):  # equivalent to SPP(k=(5, 9, 13))
        super().__init__()
        c_ = c1 // 2  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c_ * 4, c2, 1, 1)
        self.m = nn.MaxPool2d(kernel_size=k, stride=1, padding=k // 2)
        self.ema = EMA(c_ * 4, factor)  # Initialize EMA with correct channels
    ...
    def forward(self, x):
        ...
        y3 = self.m(y2)
        y = torch.cat((x, y1, y2, y3), 1)
        y = self.ema(y)  # Apply EMA
        return self.cv2(y)


######################################## SPPF with SPPF_EMA end ########################################
######################################## SPPF with SPPF_DRB start ########################################

class SPPF_DRB(SPPF):
    def __init__(self, c1, c2, k=5):  # ch_in, ch_out, kernel
        super().__init__(c1, c2, k)
        c_ = c1 // 2  # hidden channels
        self.cv2 = DilatedReparamBlock(c_ * 2, 7)  # 使用 DilatedReparamBlock 替换 cv2

######################################## SPPF with SPPF_DRB end ########################################
